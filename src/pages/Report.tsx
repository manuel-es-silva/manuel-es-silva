...
import { confirmReportSent, getReportShare, photosForRoom, recordReportShared } from '../db/queries';
...
export function Report() {
  ...
  const share = useLiveQuery(() => (propertyId ? getReportShare(propertyId, 'move-in') : undefined), [propertyId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const blob = await generateMoveInReport(property, roomsWithPhotos.filter((r) => r.photos.length > 0));
        if (cancelled) return;
        setPdfBlob(blob);
  ...
  async function handleShare() {
    ...
    const filename = `${property.address.replace(/[^a-z0-9]+/gi, '-').slice(0, 40)}-move-in-report.pdf`;
    await sharePdf(pdfBlob, filename, 'Move-in report');
    await recordReportShared(property.id, 'move-in');
  }
  ...
  function handleDownload() {
    ...
    recordReportShared(property.id, 'move-in');
  }

  async function handleConfirmSent(sent: boolean) {
    if (!property) return;
    if (sent) {
      await confirmReportSent(property.id, 'move-in');
    }
  }

  return (
    <PageShell title="Move-in report" onBack>
